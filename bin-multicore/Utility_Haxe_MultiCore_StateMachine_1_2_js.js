...
org.puremvc.haxe.multicore.utilities.statemachine.FSMInjector.prototype.createState = function(stateDef) {
	...
	var state = new org.puremvc.haxe.multicore.utilities.statemachine.State(name,entering,exiting,changed);
	{ var $it5 = stateDef.elementsNamed("transition");
	while( $it5.hasNext() ) { var transDef = $it5.next();
	{
...
org.puremvc.haxe.multicore.utilities.statemachine.FSMInjector.prototype.getStates = function() {
	if(this.stateList == null) {
		...
		{ var $it6 = this.fsm.elementsNamed("state");
		while( $it6.hasNext() ) { var stateDef = $it6.next();
		{
...
org.puremvc.haxe.multicore.utilities.statemachine.FSMInjector.prototype.inject = function() {
	...
	{ var $it7 = this.getStates().iterator();
	while( $it7.hasNext() ) { var state = $it7.next();
	{
...
org.puremvc.haxe.multicore.core.View.prototype.notifyObservers = function(notification) {
	if(this.observerMap.exists(notification.getName())) {
		...
		{ var $it8 = iterator;
		while( $it8.hasNext() ) { var observer = $it8.next();
...
org.puremvc.haxe.multicore.core.View.prototype.removeObserver = function(notificationName,notifyContext) {
	...
	{ var $it9 = observers.iterator();
	while( $it9.hasNext() ) { var observer = $it9.next();
	{
...
org.puremvc.haxe.multicore.utilities.statemachine.State = function(name,entering,exiting,changed) { if( name === $_ ) return; {
	this.name = name;
...
org.puremvc.haxe.multicore.utilities.statemachine.State.prototype.changed = null;
...
org.puremvc.haxe.multicore.utilities.statemachine.StateMachine.prototype.handleNotification = function(note) {
	...
	case "StateMachine" + "/notes/action":{
		...
		if(this.states.exists(target)) this.transitionTo(this.states.get(target),note.getBody());
	}break;
...
org.puremvc.haxe.multicore.utilities.statemachine.StateMachine.prototype.transitionTo = function(nextState,data) {
	if(nextState == null) return;
	this.canceled = false;
	if(this.getCurrentState() != null && this.getCurrentState().exiting != null) this.sendNotification(this.getCurrentState().exiting,data,nextState.name);
	if(this.canceled) {
		this.canceled = false;
		return;
	}
	if(nextState.entering != null) this.sendNotification(nextState.entering,data);
	if(this.canceled) {
		this.canceled = false;
		return;
	}
	this.setCurrentState(nextState);
	if(nextState.changed != null) this.sendNotification(this.getCurrentState().changed,data);
	this.sendNotification("StateMachine" + "/notes/changed",this.getCurrentState(),this.getCurrentState().name);
}
